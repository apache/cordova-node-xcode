...
pbxProject.prototype.addToXcVersionGroupSection = function (file) {
    ...
    if (!this.xcVersionGroupSection()[file.fileRef]) {
        this.xcVersionGroupSection()[file.fileRef] = {
            isa: 'XCVersionGroup',
            children: file.models.map(({ fileRef }) => fileRef),
...
function pbxBuildFileObj ({ fileRef, basename, settings }) {
    const obj = Object.create(null);

    obj.isa = 'PBXBuildFile';
    obj.fileRef = fileRef;
    obj.fileRef_comment = basename;
...
function pbxGroupChild ({ fileRef, basename }) {
    const obj = Object.create(null);

    obj.value = fileRef;
    obj.comment = basename;
...
function pbxShellScriptBuildPhaseObj (obj, { inputPaths, outputPaths, shellPath, shellScript }, phaseName) {
    obj.name = `"${phaseName}"`;
    obj.inputPaths = inputPaths || [];
    obj.outputPaths = outputPaths || [];
    obj.shellPath = shellPath;
    obj.shellScript = `"${shellScript.replace(/"/g, '\\"')}"`;
...
function pbxNativeTargetComment ({ name }) {
    return name;
}

function longComment ({ basename, group }) {
    return f('%s in %s', basename, group);
}
