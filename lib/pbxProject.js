...
var pbxWriter = require('./pbxWriter');
...
pbxProject.prototype.removeFromProductsPbxGroup = function (file) {
    ...
    for (var i in productsGroupChildren) {
...
pbxProject.prototype.addXCConfigurationList = function (configurationObjectsArray, defaultConfigurationName, comment) {
    ...
    var commentKey = f('%s_comment', xcConfigurationListUuid);
    ...
    for (var index = 0; index < configurationObjectsArray.length; index++) {
        ...
        var configurationCommentKey = f('%s_comment', configurationUuid);
...
pbxProject.prototype.addTargetDependency = function (target, dependencyTargets) {
    ...
    for (var index2 = 0; index2 < dependencyTargets.length; index2++) {
        var dependencyTargetUuid = dependencyTargets[index2];
        var dependencyTargetCommentKey = f('%s_comment', dependencyTargetUuid);
        var targetDependencyUuid = this.generateUuid();
        var targetDependencyCommentKey = f('%s_comment', targetDependencyUuid);
...
pbxProject.prototype.removeFromLibrarySearchPaths = function (file) {
    var configurations = nonComments(this.pbxXCBuildConfigurationSection());
    var SEARCH_PATHS = 'LIBRARY_SEARCH_PATHS';
    var config;
    var buildSettings;
    var searchPaths;
    var new_path = searchPathForFile(file, this);
...
pbxProject.prototype.addTarget = function (name, type, subfolder) {
    ...
    if (targetType === 'app_extension') {
        ...
        var watch2AppTarget = this.getTarget(producttypeForTargettype('watch2_app'));
        if (watch2AppTarget) {
            this.addBuildPhase(
                [targetName + '.appex'],
                'PBXCopyFilesBuildPhase',
    ...
    if (targetType === 'watch2_extension') {
        var watch2ExtensionTarget = this.getTarget(producttypeForTargettype('watch2_app'));
