...
function pbxWriter(contents, options) {
    ...
    if (options.dropUndefinedOrNull === undefined) {
        options.dropUndefinedOrNull = false
    }

    this.contents = contents;
    this.sync = false;
    this.indentLevel = 0;
    this.dropUndefinedOrNull = options.dropUndefinedOrNull
}
    ...
    if (proj) {
        ...
        for (key in proj) {
            ...
            if (isArray(obj)) {
                ...
                this.indentLevel--;
                this.write("};\n");
            } else if (this.dropUndefinedOrNull && (obj === undefined || obj === null)) {
                continue;
            } else if (cmt) {
                this.write("%s = %s /* %s */;\n", key, obj, cmt)
            } else {
...
pbxWriter.prototype.writeObject = function (object) {
    ...
    for (key in object) {
        ...
        if (isArray(obj)) {
            ...
            if (this.dropUndefinedOrNull && (obj === undefined || obj === null)) {
                continue;
            } else if (cmt) {
                this.write("%s = %s /* %s */;\n", key, obj, cmt)
            } else {
...
pbxWriter.prototype.writeInlineObject = function (n, d, r) {
    ...
    var inlineObjectHelper = function (name, desc, ref) {
        ...
        for (key in ref) {
            ...
            } else if (self.dropUndefinedOrNull && (obj === undefined || obj === null)) {
                continue;
            } else if (cmt) {
